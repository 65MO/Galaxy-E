...
if image=="shiny-gie-gis-region:latest":
   ...
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/SIG/?')
elif image=="shiny-geoexplorer:dev":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/SIG/?')
elif image=="shiny-hrapp:dev":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/CEFE/?')
elif image=="shiny-eolapp:dev":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/CEFE/?')
elif image=="shiny-eol2app:dev":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/CEFE/?')
elif image=="shiny-stat:dev":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/STAT/inst/app/?')
elif image=="shiny-wallace:dev":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/SIG/wallace/shiny/?')
elif image=="valentinchdock/wallace-galaxy-ie:17.09":
   notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/SIG/wallace/shiny/?')
else:
   notebook_access_url = ie_request.url_template('${PROXY_URL}?')
endif
#notebook_access_url = ie_request.url_template('${PROXY_URL}/sample-apps/?')
...
<script type="text/javascript">
        ...
        requirejs(['interactive_environments', 'plugin/shiny'], function(){
            display_spinner();
        });
