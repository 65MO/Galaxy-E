...
filtre1niveau <- function(nom_fichier = filename, dec=".",nom_fichierCouleur= color_filename,
                          ...
                          vec_col_filtre = c("REGION"),bagplot = TRUE,bagProp=c(.05,.5,.95),
                          seuilSegment=30,segmentSousSeuil=TRUE,forcageMajusculeFiltre=TRUE) {
    ...
    for(f in 1:length(vec_col_filtre)) {
        ...
        ggfiltre1niveau(d,col_abscisse,figure_abscisse,
                        col_ordonnee,figure_ordonnee,
                        figure_titre = figure_titre_f,col_filtre = col_filtre_f,
                        nomGenerique, val_filtre = NULL,figure_nom_filtre = NULL,
                        tab_figure_couleur=  subset(dCouleur,Filtre==col_filtre_f),
                        colourProtocole = colourProtocole_p,nomProtocole,
                        bagplot,bagProp=c(.05,.5,.95),
...
ggfiltre1niveau <- function(d,col_abscisse = "AB_MOYENNE",figure_abscisse = "Abondance",
                            col_ordonnee = "DIVERSITE_MOYENNE",figure_ordonnee = "Diversite",
                            figure_titre = "Referentiel papillon",col_filtre = "nom_reseau",
                            nomGenerique = "Global",
                            ...
                            seuilSegment=30,segmentSousSeuil=TRUE,forcageMajusculeFiltre=TRUE,result_dir="resultats/") {

                                        #    col_abscisse = "abond_moyenne";figure_abscisse = "Abondance"
                                        #    col_ordonnee = "diversite_moyenne";figure_ordonnee = "Diversite"
                                        #    figure_titre = "Rhooo il dechire ce graphe";col_filtre = "nom_reseau"
                                        #    nomGenerique = "Global"
                                        #   tab_figure_couleur <- subset(dCouleur,Filtre == "nom_reseau")
                                        #    nomProtocole <- "Vers de terre"
                                        #    colourProtocole <- as.character(dCouleur[dCouleur[,2]==nomProtocole,3])
    ...
    if(!(col_filtre %in% repResult)) dir.create(file.path(".",paste(result_dir,col_filtre,sep="")))
    ...
    for(m in lesModalites) {
        ...
        if(ggTableResum$groupe[2]==nomGenerique) ggTableResum <- ggTableResum[c(2,1),]
        ...
        if(!(m %in% repResult)) dir.create(paste(nomRep1,m,sep=""))
        nomRep <- paste(nomRep1,m,"/",sep="") 
        
        
        if(!is.null(nomProtocole)) {
            repResult <- dir(nomRep)
            if(!(nomProtocole %in% repResult)) dir.create(paste(nomRep,nomProtocole,sep=""))
        ...
        if(!(is.null(tab_figure_couleur))) gg <- gg + scale_colour_manual(values = figure_couleur,name = "") + scale_fill_manual(values = figure_couleur,name = "",guide=FALSE)
...
filtre1niveau(nom_fichier=filename,nom_fichierCouleur=color_filename)
...
gglocal <- function(d,col_abscisse = "AB_MOYENNE",figure_abscisse = "Abondance",
                    col_ordonnee = "DIVERSITE_MOYENNE",figure_ordonnee = "Diversite",
                    figure_titre = "Graphe referentiel",col_filtre = "NOM_RESEAU",
                    nomGenerique = "GLOBAL",col_sousGroup = "PARCELLEID",
                    val_filtre = NULL,figure_nom_filtre = NULL,
                    tab_figure_couleur= NULL,
                    colourProtocole = NULL,nomProtocole = NULL,couleurLocal="#f609c1",
                    bagplot = TRUE,bagProp=c(.05,.5,.95),
                    seuilSegment=30,segmentSousSeuil=TRUE,forcageMajusculeFiltre=TRUE,forcageMajusculeSousGroupe=TRUE) {
    ...
    if(forcageMajusculeFiltre) d$groupe <- toupper(d$groupe)
    if(forcageMajusculeSousGroupe) d$sousGroup <- toupper(d$sousGroup)
