...
from galaxy.tools.actions.shiny import ShinyToolAction
...
class ShinyTool( OutputParameterJSONTool ):
    """
    Alternate implementation of Tool for shiny tools -- those that
    allow the user to deploy a shiny app with a data from history.
    """
    tool_type = 'shiny'
    default_tool_action = ShinyToolAction

    def _build_GALAXY_URL_parameter( self ):
        return ToolParameter.build( self, ElementTree.XML( '<param name="GALAXY_URL" type="baseurl" value="/tool_runner?tool_id=%s" />' % self.id ) )

    def parse_inputs( self, tool_source ):
        super( ShinyTool, self ).parse_inputs( tool_source )
    #     Open all data_source tools in _top.
        self.target = '_top'
        if 'GALAXY_URL' not in self.inputs:
           self.inputs[ 'GALAXY_URL' ] = self._build_GALAXY_URL_parameter()
...
for tool_class in [ Tool, SetMetadataTool, OutputParameterJSONTool,
                    DataManagerTool, DataSourceTool, ShinyTool, AsyncDataSourceTool,
