...
IdCorrect$IdProb=mapply(FUN=function(w,x,y) if((y>0)&(y<1000)) {(exp(y*w+x)/(1+exp(y*w+x)))}else{w}
  ,IdCorrect$IdScore,RefSeuil$Int,RefSeuil$Pente)
...
print(paste(i,length(subset(IdCorrect$ConfV,IdCorrect$ConfV!=""))))
