...
#for (i in 1:length(inputest)){args=c(inputest[i],"refPF.csv","SpeciesList.csv")
print(args)
...
{
  print("Protocole non conforme, ce script doit être lancé uniquement pour un protocole Point Fixe")
}else{
  refPF=fread(args[2])
  ...
  #get date-night
  pourDateNuit=IdC2$TimeNum-16*3600 #bricolage-décalage de 12 heures pour ramener à la date du début de nuit
  ...
  write.csv2(SummPartN,paste0(substr(args[1],1,nchar(args[1])-9),"-nightly.csv"),row.names=F)
  ...
  write.csv2(SummPartH,paste0(substr(args[1],1,nchar(args[1])-9),"-hourly.csv"),row.names=F)
