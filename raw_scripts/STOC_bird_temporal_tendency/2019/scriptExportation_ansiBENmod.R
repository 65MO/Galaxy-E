...
ca.id_carre, annee;",sep="")
        ...
        dToutCarreAn <- dbGetQuery(con, queryCarreAn)### recup tous les pairs carres/annee ou une sps a �t� observ� (donc si aucune obs, le carre normalement saute sauf si selection de plusieurs esp avec une sur ces carres)
        ...
        if(nrow(dCarreAbs)>0) {
            ...
            dspAbs <- as.data.frame(lapply(dsp, rep,each= nbCarreAbs))#### chaque element de nbCarreAbs est r�p�t� x fois
            dCarreAbs <- cbind(dCarreAbs2,dspAbs)#### tu r��cris sur dCarreAbs ?
